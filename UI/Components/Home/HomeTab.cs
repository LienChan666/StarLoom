...
internal sealed class HomeTab
{
    ...
    private void DrawMainContent()
    {
        ...
        using var _ = GamePanelStyle.BeginPanel("##HomeFlowShell", new Vector2(rightWidth, availableSize.Y), GamePanelStyle.PanelBorder);
        GamePanelStyle.DrawPanelHeader("兑换编排区", "先搜索需要的工票物品，再在下方维护当前兑换队列。\n整个操作流保持在同一块面板内完成。");

        var contentHeight = ImGui.GetContentRegionAvail().Y;
        var verticalSpacing = ImGui.GetStyle().ItemSpacing.Y;

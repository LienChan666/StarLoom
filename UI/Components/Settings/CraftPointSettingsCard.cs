...
internal sealed class CraftPointSettingsCard
{
    ...
    public void Draw()
    {
        ...
        var preview = hasValidConfiguredPoint
            ...
            : configuredPoint.IsInn
                ? _plugin.GetText("settings.craft_point.inn")
        ...
        GamePanelStyle.DrawSettingLabel(_plugin.GetText("settings.craft_point.list"));
        ImGui.TableSetColumnIndex(1);
        if (ImGui.Button($"{_plugin.GetText("settings.craft_point.refresh_button")}##RefreshReturnPoints", new Vector2(140f, 0f)))
        ...
        ImGui.TableNextRow();
        ImGui.TableSetColumnIndex(0);
        GamePanelStyle.DrawSettingLabel(_plugin.GetText("settings.craft_point.list_status"));
        ImGui.TableSetColumnIndex(1);
        ImGui.TextWrapped(_plugin.GetText(_returnPointsCacheStatusKey, _returnPointsCacheStatusArgs));

        ImGui.TableNextRow();
        ImGui.TableSetColumnIndex(0);
        GamePanelStyle.DrawSettingLabel(_plugin.GetText("settings.craft_point.default_point"));
        ImGui.TableSetColumnIndex(1);
        ...
        ImGui.TableNextRow();
        ImGui.TableSetColumnIndex(0);
        GamePanelStyle.DrawSettingLabel(_plugin.GetText("settings.craft_point.current_status"));
        ImGui.TableSetColumnIndex(1);
        if (configuredPoint.IsInn)
        {
            ImGui.TextWrapped(_plugin.GetText("settings.craft_point.current_inn"));
        }
        else if (!_returnPointsCacheLoaded)
        {
            ImGui.TextWrapped(_plugin.GetText("settings.craft_point.current_saved_unverified", configuredPoint.DisplayName));
        }
        else if (!hasValidConfiguredPoint)
        {
            ImGui.TextColored(GamePanelStyle.Danger, _plugin.GetText("settings.craft_point.current_invalid"));
        }
        else
        {
            ImGui.TextWrapped(_plugin.GetText("settings.craft_point.current_configured", GetPointLabel(resolvedPoint!)));
        }
    ...
    private string GetPointLabel(HousingReturnPoint point)
        => point.IsInn ? _plugin.GetText("settings.craft_point.inn") : point.DisplayName;
